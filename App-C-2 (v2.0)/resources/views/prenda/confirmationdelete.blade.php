...
<body>
    <div class="card">
        ...
        <form action="{{ route('prendas.destroy',$id) }}" method="POST">
            <div class="card-button-wrapper">
                <a class="btn btn-sm btn-success" href="{{ route('prendas.index') }}"><i class="fa fa-fw fa-edit"></i> {{ __('Cancelar') }}</a>
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-fw fa-trash"></i> {{ __('Borrar') }}</button>
            </div>
        </form>
        <a href="{{route('prendas.index')}}"><button class="exit-button">
