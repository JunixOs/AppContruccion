...
<body>
    <div class="card">
        <div class="card-content">
            <p class="card-heading">¿Esta seguro que desea eliminar la publicacion?</p>
            <p class="card-description">Esta accion es irreversible</p>
        </div>
        <form action="{{ route('juguetes.destroy',$id) }}" method="POST">
            <div class="card-button-wrapper">
                <a class="btn btn-sm btn-success" href="{{ route('juguetes.index') }}"><i class="fa fa-fw fa-edit"></i> {{ __('Cancelar') }}</a>
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-sm"><i class="fa fa-fw fa-trash"></i> {{ __('Borrar') }}</button>
            </div>
        </form>
        <a href="{{route('juguetes.index')}}"><button class="exit-button">
